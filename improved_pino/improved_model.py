...
class ImprovedPINO(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_height_skip:
            output_in_channels = width + width // 4
        else:
            output_in_channels = width
    ...
    def forward(self, x):
        try:
            ...
            if x.shape[1] != self.in_channels:
                raise ValueError(f"Input channel mismatch: expected {self.in_channels}, actual {x.shape[1]}")
            ...
            for i, fno_layer in enumerate(self.fno_layers):
                ...
                if torch.isnan(x).any() or torch.isinf(x).any():
                    ...
                    x = torch.nan_to_num(x, nan=0.0, posinf=self.max_norm, neginf=-self.max_norm)
            if self.use_height_skip:
                x = torch.cat([x, height_skip], dim=1)
...
if __name__ == "__main__":
    ...
    print("✓ Model forward pass normal")
    ...
    print("\n✓ All tests passed! Model modification completed:")
    ...
    print("  - Focus shifted to physical constraint implementation")
