    ...
    def forward(self, x):
        try:
            ...
            if x.shape[1] != self.in_channels:
                raise ValueError(f"输入通道数不匹配: 期望 {self.in_channels}, 实际 {x.shape[1]}")
...
if __name__ == "__main__":
    ...
    print(f"高分辨率输出形状: {hr_output.shape}")
    complexity = model.compute_model_complexity()
    print(f"模型复杂度: {complexity}")
    print("\n✓ 所有测试通过！模型修改完成：")
    print("  - 输入通道确保为14")
    print("  - 使用普通输出头替代物理感知头")
    print("  - 完全移除FNO层Dropout")
    print("  - 移除物理感知头相关的硬编码约束")
    print("  - 重点转向物理约束的实现")
