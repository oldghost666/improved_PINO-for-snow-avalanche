...
class ImprovedAvalancheDataset(Dataset):
    def __init__(
        ...
        print(f"Improved dataset initialization completed:")
        ...
        print(f"  - Grouped by time scale: {self.group_by_time_scale}")
    def _load_data_info(self):
        with h5py.File(self.h5_file_path, 'r') as f:
            ...
            if self.tile_ids:
                ...
                if invalid_tiles:
                    raise ValueError(f"Specified tiles do not exist in the H5 file: {invalid_tiles}")
            ...
            try:
                ...
                print(f"🔄 Loading normalization parameters from global config (version: {loaded_params.get('version', 'unknown')})")
            except Exception as e:
                print(f"⚠️  Error: Unable to get normalization parameters from global config - {e}")
                raise RuntimeError(f"Normalization parameters must be obtained through the global configuration chain. Please check the global_data_config.py configuration.")
            ...
            try:
                ...
                print(f"✅ Successfully loaded normalization parameters (version: {loaded_params['version']})")
                ...
                print(f"   - Physics parameter range: μ₀[{self.normalization_stats['mu_0_min']:.3f}, {self.normalization_stats['mu_0_max']:.3f}], ξ₀[{self.normalization_stats['xi_0_min']:.0f}, {self.normalization_stats['xi_0_max']:.0f}], ρ[{self.normalization_stats['rho_min']:.0f}, {self.normalization_stats['rho_max']:.0f}]")
                loaded_from_external = True
            except Exception as e:
                print(f"⚠️  Error: Unable to process normalization parameters - {e}")
                raise RuntimeError(f"Normalization parameter processing failed. Please check the integrity of the global configuration chain.")
            if loaded_from_external:
                print(f"✅ Successfully loaded normalization parameters from external file, ensuring global consistency")
                return
            else:
                raise RuntimeError("Unable to load specified normalization parameters file. Please check file path and format.")
    def _build_data_indices(self):
        ...
        print(f"  - Data index construction completed:")
        print(f"    Total samples: {len(self.data_indices)}")
        ...
        print(f"    Number of samples per time pair: {[len(group['samples']) for group in self.time_pair_groups[:5]]}...")
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        if self.normalize:
            ...
            if h_current.size > 0 and (idx == 0 or idx % 1000 == 0):
                print(f"[Sample {idx}] Height normalization - original range: [{np.min(h_current):.4f}, {np.max(h_current):.4f}]")
                print(f"[Sample {idx}] Height normalization - using parameters: mean={height_mean:.4f}, std={height_std:.4f}")
            h_current = (h_current - height_mean) / (height_std + 1e-8)
            h_target = (h_target - height_mean) / (height_std + 1e-8)
            if h_current.size > 0 and (idx == 0 or idx % 1000 == 0):
                print(f"[Sample {idx}] Height normalization - normalized range: [{np.min(h_current):.4f}, {np.max(h_current):.4f}]")
    ...
    def load_normalization_stats(self, load_path: str):
        ...
        print(f"Normalization statistics loaded from {load_path}")
...
def create_improved_dataloader(
    ...
    if group_by_time_pair and hasattr(dataset, 'time_pair_groups'):
        ...
        print("Warning: group_by_time_scale is deprecated, use group_by_time_pair instead")
        return DataLoader(
            dataset,
            batch_size=batch_size,
            shuffle=shuffle,
