...
class ImprovedAvalancheDataset(Dataset):
    def __init__(
        ...
        group_by_time_scale: bool = False,  
        time_scale_tolerance: float = 0.1   
    ):
        self.h5_file_path = h5_file_path
        self.tile_ids = tile_ids
        self.sequence_length = sequence_length
        self.prediction_steps = prediction_steps
        self.mode = sampling_mode if sampling_mode is not None else mode
        self.normalize = normalize
        self.dx = dx
        self.dy = dy
        self.dt = dt
        self.boundary_condition = boundary_condition  
        self.group_by_time_scale = group_by_time_scale  
        self.time_scale_tolerance = time_scale_tolerance  
        self.in_dim = 6  
        self.static_dim = 4  
        self.physics_dim = 4  
        self.out_dim = 3  
        ...
        print(f"改进版数据集初始化完成:")
        print(f"  - 工况数量: {len(self.tile_ids)}")
        print(f"  - 总样本数: {len(self.data_indices)}")
        print(f"  - 输入通道: 6 (动态+坐标) + 4 (静态) + 4 (物理参数) = 14")
        print(f"  - 输出通道: 3 (h, vx, vy)")
        print(f"  - 边界条件: {self.boundary_condition}")
        print(f"  - 按时间尺度分组: {self.group_by_time_scale}")
    def _load_data_info(self):
        with h5py.File(self.h5_file_path, 'r') as f:
            available_tiles = [key for key in f.keys() if key != 'metadata']
            print(f"H5文件中可用工况: {available_tiles}")
            if self.tile_ids:
                invalid_tiles = [tile for tile in self.tile_ids if tile not in available_tiles]
                if invalid_tiles:
                    raise ValueError(f"指定的工况不存在于H5文件中: {invalid_tiles}")
                print(f"使用指定的 {len(self.tile_ids)} 个工况: {self.tile_ids}")
            else:
                self.tile_ids = available_tiles
                print(f"使用所有 {len(self.tile_ids)} 个工况: {self.tile_ids}")
            try:
                ...
                print(f"🔄 从全局配置加载归一化参数 (版本: {loaded_params.get('version', 'unknown')})")
            except Exception as e:
                print(f"⚠️  错误：无法从全局配置获取归一化参数 - {e}")
                raise RuntimeError(f"必须通过全局配置链条获取归一化参数，请检查global_data_config.py配置")
            ...
            try:
                ...
                print(f"✅ 成功加载归一化参数 (版本: {loaded_params['version']})")
                print(f"   - 高度范围: [{self.normalization_stats['heights_min']:.4f}, {self.normalization_stats['heights_max']:.4f}]")
                print(f"   - 速度均值/标准差: x({self.normalization_stats['velocity_x_mean']:.4f}/{self.normalization_stats['velocity_x_std']:.4f}), y({self.normalization_stats['velocity_y_mean']:.4f}/{self.normalization_stats['velocity_y_std']:.4f})")
                print(f"   - DEM范围: [{self.normalization_stats['dem_min']:.1f}, {self.normalization_stats['dem_max']:.1f}]")
                print(f"   - 物理参数范围: μ₀[{self.normalization_stats['mu_0_min']:.3f}, {self.normalization_stats['mu_0_max']:.3f}], ξ₀[{self.normalization_stats['xi_0_min']:.0f}, {self.normalization_stats['xi_0_max']:.0f}], ρ[{self.normalization_stats['rho_min']:.0f}, {self.normalization_stats['rho_max']:.0f}]")
                loaded_from_external = True
            except Exception as e:
                print(f"⚠️  错误：无法处理归一化参数 - {e}")
                raise RuntimeError(f"归一化参数处理失败，请检查全局配置链条的完整性")
            if loaded_from_external:
                print(f"✅ 成功从外部文件加载归一化参数，确保全局一致性")
                return
            else:
                raise RuntimeError("无法加载指定的归一化参数文件，请检查文件路径和格式")
    def _build_data_indices(self):
        ...
        with h5py.File(self.h5_file_path, 'r') as f:
            for tile_id in self.tile_ids:
                if tile_id in f:
                    ...
                    for t in range(num_timesteps - self.prediction_steps):
                        input_time = times[t]
                        target_time = times[t + self.prediction_steps]
        ...
        print(f"  - 数据索引构建完成:")
        print(f"    总样本数: {len(self.data_indices)}")
        print(f"    时间对数量: {len(self.time_pair_groups)}")
        print(f"    时间范围: {sorted_time_pairs[0][0]:.1f}s → {sorted_time_pairs[-1][1]:.1f}s")
        print(f"    每个时间对的样本数: {[len(group['samples']) for group in self.time_pair_groups[:5]]}...")
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        H, W = dem.shape
        h_current = heights[time_idx]  
        vx_current = velocity_x[time_idx]  
        vy_current = velocity_y[time_idx]  
        ...
        t_norm = (times[time_idx] - t_min) / (t_max - t_min)  
        T = np.full((H, W), t_norm)
        if self.normalize:
            height_mean = self.normalization_stats['heights_mean']
            height_std = self.normalization_stats['heights_std']
            if h_current.size > 0 and (idx == 0 or idx % 1000 == 0):
                print(f"[样本{idx}] 高度归一化 - 原始值范围: [{np.min(h_current):.4f}, {np.max(h_current):.4f}]")
                print(f"[样本{idx}] 高度归一化 - 使用参数: mean={height_mean:.4f}, std={height_std:.4f}")
            h_current = (h_current - height_mean) / (height_std + 1e-8)
            h_target = (h_target - height_mean) / (height_std + 1e-8)
            if h_current.size > 0 and (idx == 0 or idx % 1000 == 0):
                print(f"[样本{idx}] 高度归一化 - 归一化后范围: [{np.min(h_current):.4f}, {np.max(h_current):.4f}]")
        ...
        mu_0_val = physical_params['mu_0']
        xi_0_val = physical_params['xi_0']
        rho_val = physical_params['rho']
        cohesion_val = physical_params['cohesion']
        if self.normalize:
            mu_0_min = self.normalization_stats['mu_0_min']
            mu_0_max = self.normalization_stats['mu_0_max']
            mu_0_range = mu_0_max - mu_0_min
            if mu_0_range > 1e-8:
                mu_0_val = (mu_0_val - mu_0_min) / mu_0_range
            else:
                mu_0_val = 0.0
            mu_0_val = np.clip(mu_0_val, 0.0, 1.0)
            xi_0_min = self.normalization_stats['xi_0_min']
            xi_0_max = self.normalization_stats['xi_0_max']
            xi_0_range = xi_0_max - xi_0_min
            if xi_0_range > 1e-8:
                xi_0_val = (xi_0_val - xi_0_min) / xi_0_range
            else:
                xi_0_val = 0.0
            xi_0_val = np.clip(xi_0_val, 0.0, 1.0)
            rho_min = self.normalization_stats['rho_min']
            rho_max = self.normalization_stats['rho_max']
            rho_range = rho_max - rho_min
            if rho_range > 1e-8:
                rho_val = (rho_val - rho_min) / rho_range
            else:
                rho_val = 0.0
            rho_val = np.clip(rho_val, 0.0, 1.0)
            ...
            if cohesion_range > 1e-8:
                cohesion_val = (cohesion_val - cohesion_min) / cohesion_range
            else:
                cohesion_val = 0.0
            cohesion_val = np.clip(cohesion_val, 0.0, 1.0)
        mu_0_field = np.full((H, W), mu_0_val)
        xi_0_field = np.full((H, W), xi_0_val)
        rho_field = np.full((H, W), rho_val)
        cohesion_field = np.full((H, W), cohesion_val)
        ...
        height_weight = np.ones_like(h_target)
        thin_snow_mask = (h_target > 0.05) & (h_target < 0.5)  
        height_weight[thin_snow_mask] = 2.0  
        sample = {
            : torch.from_numpy(input_dynamic).float(),
            : torch.from_numpy(input_static).float(),
            : torch.from_numpy(input_physics).float(),  
            : torch.from_numpy(target).float(),
            : torch.from_numpy(height_weight).float(),  
            : {k: torch.tensor(v, dtype=torch.float32) for k, v in physical_params.items()},
            : torch.tensor(self.dx, dtype=torch.float32),
            : torch.tensor(self.dy, dtype=torch.float32),
            : torch.tensor(self.dt, dtype=torch.float32),
            : tile_id,
            : time_idx,
            : torch.tensor(sample_info.get('time_scale', 0.0), dtype=torch.float32),  
    ...
    def denormalize_height(self, normalized_height):
        ...
        if isinstance(normalized_height, np.ndarray) and normalized_height.size > 0 and self._denorm_print_count % 500 == 0:
            print(f"[反归一化{self._denorm_print_count}] 高度反归一化 - 归一化值范围: [{np.min(normalized_height):.4f}, {np.max(normalized_height):.4f}]")
            print(f"[反归一化{self._denorm_print_count}] 高度反归一化 - 使用参数: mean={height_mean:.4f}, std={height_std:.4f}")
    ...
    def denormalize_velocity(self, normalized_velocity_x, normalized_velocity_y):
        ...
        if isinstance(normalized_velocity_x, np.ndarray) and normalized_velocity_x.size > 0 and self._vel_denorm_print_count % 500 == 0:
            print(f"[速度反归一化{self._vel_denorm_print_count}] vx归一化值范围: [{np.min(normalized_velocity_x):.4f}, {np.max(normalized_velocity_x):.4f}]")
            print(f"[速度反归一化{self._vel_denorm_print_count}] vy归一化值范围: [{np.min(normalized_velocity_y):.4f}, {np.max(normalized_velocity_y):.4f}]")
            print(f"[速度反归一化{self._vel_denorm_print_count}] 使用参数: vx_std={velocity_x_std:.4f}, vx_mean={velocity_x_mean:.4f}, vy_std={velocity_y_std:.4f}, vy_mean={velocity_y_mean:.4f}")
    ...
    def load_normalization_stats(self, load_path: str):
        ...
        print(f"归一化统计信息已从 {load_path} 加载")
class TimePairGroupedSampler:
    def __init__(self, dataset, batch_size, shuffle=True):
        ...
        for i, group in enumerate(self.time_pair_groups[:3]):
            print(f"  - 时间对{i+1}: {group['input_time']:.1f}s → {group['target_time']:.1f}s, 样本数: {len(group['samples'])}")
    def __iter__(self):
        import random
...
def create_improved_dataloader(
    ...
    if group_by_time_pair and hasattr(dataset, 'time_pair_groups'):
        ...
        print(f"使用标准数据加载器: 批次大小={batch_size}, 打乱={shuffle}")
        return DataLoader(
            dataset,
            batch_size=batch_size,
            shuffle=shuffle,
