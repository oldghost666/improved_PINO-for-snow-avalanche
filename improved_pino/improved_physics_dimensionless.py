...
class DimensionlessPhysicsLoss(nn.Module):
    def __init__(self, config: Dict[str, Any], global_data_config, device='cuda'):
        ...
        logging.info(f"无量纲化物理损失初始化完成:")
        logging.info(f"  空间尺度: dx={self.dx}m, dy={self.dy}m")
        logging.info(f"  时间尺度: dt={self.dt}s")
        logging.info(f"  高度特征尺度: H_char={self.H_char:.3f}m")
        logging.info(f"  速度特征尺度: U_char={self.U_char:.3f}m/s")
        logging.info(f"  加速度特征尺度: A_char={self.A_char:.3f}m/s²")
    ...
    def _initialize_characteristic_scales(self):
        try:
            ...
            logging.info(f"  Froude数={self.Froude:.3f}")
        except Exception as e:
            logging.warning(f"获取统计参数失败，使用默认值: {e}")
            self.H_char = 1.0
            self.U_char = 5.0
            self.A_char = self.g
            self.Froude = 1.0
    def compute_dimensionless_physics_loss(self, 
        ...
        try:
            ...
            logging.error(f"无量纲化物理损失计算失败: {e}")
    ...
    def _prepare_physics_data(self, x_batch: torch.Tensor, predictions: torch.Tensor, 
        ...
        if x_batch.shape[1] != 14:
            raise ValueError(f"期望14通道输入，但得到{x_batch.shape[1]}通道。请确保输入包含所有物理参数。")
    ...
    def _compute_physics_constraints(self, pde_residuals: Dict[str, torch.Tensor], 
        ...
        if torch.isnan(total_physics_loss) or torch.isinf(total_physics_loss):
            logging.warning("物理损失出现NaN或Inf，返回零损失")
            return torch.tensor(0.0, device=self.device, requires_grad=True)
    ...
    def compute_physics_loss(self, x_batch, pred_norm, input_norm_stats, target_norm_stats, step=None, 
        ...
        try:
            ...
            logging.warning(f"无量纲化物理损失计算失败: {e}")
            return torch.tensor(0.0, device=self.device, requires_grad=True)
