...
class GlobalDataConfig:
    def __init__(self, h5_file_path: str = None):
        if h5_file_path is None:
            raise ValueError("h5_file_path parameter is required and cannot be a default value. Please specify the data file path explicitly in the configuration file.")
    ...
    def _load_normalization_params(self):
        try:
            ...
            print(f"✅ Successfully loaded normalization parameters file: {self.normalization_file_path}")
            print(f"📊 Normalization strategy (optimized based on 828.txt):")
            print(f"   - Heights (heights): Standard Normalization ✅ Core Modification")
            ...
            if missing_fields:
                print(f"⚠️  Warning: Missing the following normalization parameters: {missing_fields}")
        except Exception as e:
            print(f"❌ Failed to load normalization parameters file: {e}")
            ...
            for key in key_params:
                if key in json_params and key in self.normalization_params:
                    ...
                    if abs(json_val - config_val) > 1e-6:
                        print(f"⚠️  Parameter inconsistency: {key} - JSON: {json_val}, Config: {config_val}")
                        all_consistent = False
                    else:
                        print(f"✅ Parameter consistency: {key} = {json_val}")
                else:
                    print(f"❌ Missing parameter: {key}")
                    all_consistent = False
            if all_consistent:
                print("✅ All key parameters verified successfully")
            else:
                print("⚠️  Parameter inconsistency exists")
        except Exception as e:
    ...
    def update_data_loading_config(self, **kwargs):
        for key, value in kwargs.items():
            if key in self.data_loading_config:
                ...
                print(f"✅ Updated data loading config: {key} = {value}")
            else:
                print(f"⚠️  Warning: Unknown data loading configuration item: {key}")
