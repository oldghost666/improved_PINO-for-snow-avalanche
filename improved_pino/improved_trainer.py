...
class OptimizedLossFunction(nn.Module):
    def __init__(self, config: Dict):
        ...
        if 'h5_file_path' not in data_config:
            raise ValueError("Missing required 'data.h5_file_path' parameter in configuration file")
...
class ProgressiveTrainer:
    ...
    def setup_realtime_visualization(self):
        try:
            ...
            self.axes = None
            self.lines = None
            self.logger.info("✓ Loss data logging enabled (no real-time display)")
    ...
    def update_realtime_plot(self, epoch: int, train_metrics: Dict, val_metrics: Dict):
        ...
        try:
            ...
            self.logger.warning(f"Failed to record loss data: {e}")
    def save_loss_curves(self, save_path: str = None):
        ...
        try:
            ...
            fig.suptitle('PINO Avalanche Model Training Loss Curves', fontsize=16, fontweight='bold')
            epochs = self.detailed_loss_history['epoch']
            ax1 = axes[0, 0]
            ax1.set_title('Total Loss Comparison (Train vs Val)', fontweight='bold')
            ...
            ax3.legend()
            ax4 = axes[1, 1]
            ax4.set_title('Loss Weight Change', fontweight='bold')
            ...
            plt.close(fig)
            self.logger.info(f"Loss curve plot saved to: {save_path}")
            import json
    ...
    def create_model_and_components(self) -> Tuple:
        ...
        self.logger.info(f"📋 Physics module selection: {physics_module_type}")
        self.logger.info("=" * 60)
        if physics_module_type == 'dimensionless':
            try:
                ...
                self.logger.info("✅ Successfully initialized: DimensionlessPhysicsLoss (Dimensionless physics loss module)")
                self.logger.info("📊 Features: Dimensionless processing based on physical scales, resolving dimensional inconsistency")
                self.logger.info("🎯 Advantages: Spatial derivative physical scale correction, statistical parameter characteristic scale normalization, dimensional consistency of Shallow Water Equations")
            except Exception as e:
                self.logger.warning(f"❌ Dimensionless physics loss module initialization failed: {e}")
                self.logger.warning("🔄 Automatically falling back to the old improved physics loss module")
                physics_module_type = 'improved'
        if physics_module_type == 'improved':
            raise RuntimeError("Dimensionless physics loss module initialization failed, please check configuration file and dependencies")
        self.logger.info("=" * 60)
    ...
    def create_data_loaders(self) -> Tuple:
        if 'group_by_time_scale' in self.config['data']:
            self.logger.warning("⚠️ Deprecated configuration field 'group_by_time_scale' detected. Please use 'group_by_time_pair' instead.")
            self.logger.info("✓ Automatically using 'group_by_time_pair' strategy")
        ...
        self.logger.info(f"Training data loading strategy: Time pair grouping={self.config['data'].get('group_by_time_pair', True)}")
        ...
        for batch_idx, batch_data in enumerate(self.train_loader):
            ...
            if weights['boundary_weight'] > 0:
                try:
                    ...
                    self.logger.warning(f"Boundary loss calculation failed: {e}")
                    boundary_loss = torch.tensor(0.0, device=self.device)
            total_batch_loss = (
    ...
    def train(self):
        ...
        for epoch in range(self.config['training']['epochs']):
            ...
            if self.patience_counter >= self.config['training']['patience']:
                self.logger.info(f"Early stopping triggered, training stopped at epoch {epoch+1}")
                break
